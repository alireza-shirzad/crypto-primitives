use crate::sponge::rescue::grain_lfsr::RescueGrainLFSR;
...
/// An entry in the default Rescue parameters
pub struct RescueDefaultConfigEntry {
...
impl RescueDefaultConfigEntry {
    /// Create an entry in `RescueDefaultConfig`.
    pub const fn new(
        rate: usize,
        alpha: u64,
        rounds: usize,
...
/// A trait for default Rescue parameters associated with a prime field
pub trait RescueDefaultConfig<const N: usize>: FpConfig<N> {
    ...
    const PARAMS_OPT_FOR_CONSTRAINTS: [RescueDefaultConfigEntry; 1];

    /// An array of the parameters optimized for weights
    /// (rate, alpha, full_rounds, partial_rounds, skip_matrices)
    /// for rate = 2, 3, 4, 5, 6, 7, 8
    const PARAMS_OPT_FOR_WEIGHTS: [RescueDefaultConfigEntry; 1];
}

/// A field with Rescue parameters associated
pub trait RescueDefaultConfigField: PrimeField {
    /// Obtain the default Rescue parameters for this rate and for this prime field,
    /// with a specific optimization goal.
    fn get_default_rescue_parameters(
        rate: usize,
        optimized_for_weights: bool,
    ) -> Option<RescueConfig<Self>>;
}

/// Internal function that uses the `RescueDefaultConfig` to compute the Rescue parameters.
pub fn get_default_rescue_parameters_internal<P: RescueDefaultConfig<N>, const N: usize>(
    rate: usize,
    optimized_for_weights: bool,
) -> Option<RescueConfig<Fp<P, N>>> {
    ...
    for param in params_set.iter() {
        if param.rate == rate {
            let (arc, mds) = find_rescue_ark_and_mds::<Fp<P, N>>(
            ...
            return Some(RescueConfig {
                rounds: param.rounds,
                alpha: param.alpha,
                alpha_inv: Fp::<P, N>::inverse(&Fp::<P,N>::from(param.alpha)).unwrap().into(),
                arc,
                mds,
                rate: param.rate,
                capacity: 1,
...
/// Internal function that computes the ark and mds from the Rescue Grain LFSR.
pub fn find_rescue_ark_and_mds<F: PrimeField>(
    prime_bits: u64,
    rate: usize,
    rounds: u64,
    skip_matrices: u64,
) -> (Vec<Vec<F>>, Vec<Vec<F>>) {
    let mut lfsr = RescueGrainLFSR::new(
        false,
        prime_bits,
        (rate + 1) as u64,
        rounds,
    ...
    let mut ark = Vec::<Vec<F>>::with_capacity(rounds as usize);
    for _ in 0..rounds {
...
impl<const N: usize, P: RescueDefaultConfig<N>> RescueDefaultConfigField for Fp<P, N> {
    fn get_default_rescue_parameters(
        rate: usize,
        optimized_for_weights: bool,
    ) -> Option<RescueConfig<Self>> {
        get_default_rescue_parameters_internal::<P, N>(rate, optimized_for_weights)
    }
}

// #[cfg(test)]
// mod test {
//     use crate::sponge::rescue::RescueDefaultConfigField;
//     use crate::sponge::test::*;
//     use ark_ff::MontFp;

//     #[test]
//     fn bls12_381_fr_rescue_default_parameters_test() {
//         // constraints
//         let constraints_rate_2 = Fr::get_default_rescue_parameters(2, false).unwrap();
//         assert_eq!(
...
//         let constraints_rate_3 = Fr::get_default_rescue_parameters(3, false).unwrap();
//         assert_eq!(
...
//         let constraints_rate_4 = Fr::get_default_rescue_parameters(4, false).unwrap();
//         assert_eq!(
...
//         let constraints_rate_5 = Fr::get_default_rescue_parameters(5, false).unwrap();
//         assert_eq!(
...
//         let constraints_rate_6 = Fr::get_default_rescue_parameters(6, false).unwrap();
//         assert_eq!(
...
//         let constraints_rate_7 = Fr::get_default_rescue_parameters(7, false).unwrap();
//         assert_eq!(
...
//         let constraints_rate_8 = Fr::get_default_rescue_parameters(8, false).unwrap();
//         assert_eq!(
...
//         // weights
//         let weights_rate_2 = Fr::get_default_rescue_parameters(2, true).unwrap();
//         assert_eq!(
...
//         let weights_rate_3 = Fr::get_default_rescue_parameters(3, true).unwrap();
//         assert_eq!(
...
//         let weights_rate_4 = Fr::get_default_rescue_parameters(4, true).unwrap();
//         assert_eq!(
...
//         let weights_rate_5 = Fr::get_default_rescue_parameters(5, true).unwrap();
//         assert_eq!(
...
//         let weights_rate_6 = Fr::get_default_rescue_parameters(6, true).unwrap();
//         assert_eq!(
...
//         let weights_rate_7 = Fr::get_default_rescue_parameters(7, true).unwrap();
//         assert_eq!(
...
//         let weights_rate_8 = Fr::get_default_rescue_parameters(8, true).unwrap();
//         assert_eq!(
